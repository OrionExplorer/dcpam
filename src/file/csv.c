...
char** _CSV_parse_line( const char* line, const char* delimiter, int *dst_count ) {
    int     i = 0;
    char    token[ 1024 ];
    int     token_pos = 0;
    ...
    do {
        ...
        if( !in_double_quotes && ( line[ i ] == delimiter[ 0 ] || line[ i ] == '\n' ) ) {
            ...
            tmp = realloc( tmp, ( val_count + 1 ) * sizeof * tmp );
...
int CSV_FILE_load( CSV_FILE* dst, const char* filename, clc* csv_load_callback, void *data_ptr1, void *data_ptr2, LOG_OBJECT* log ) {
    ...
    while( fgets( buf, 1024, csv_f ) ) {
        row_count++;
        if( row_count == 1 ) {
            columns = _CSV_parse_line( buf, dst->delimiter, &csv_columns->field_count );
        } else {
            if( csv_columns->field_count > 0 ) {
                ...
                row_values = _CSV_parse_line( buf, dst->delimiter, NULL );
