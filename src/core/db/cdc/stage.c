...
void CDC_StageGeneric( DB_SYSTEM_CDC_STAGE *stage, DB_SYSTEM_CDC_STAGE_QUERY *stage_element, DATABASE_SYSTEM_DB *db, DB_QUERY *data );
    ...
    if( stage ) {
        ...
        for( int i = 0; i < data->row_count; i++ ) {
            ...
            if( q_values_len > 0 ) {
                ...
                int query_ret = DB_exec( &APP.DB, stage_element->sql, stage_element->sql_len, &sql_res, ( const char* const* )q_values, q_values_len, q_lengths, q_formats, NULL );
...
void DB_CDC_StageInserted( DB_SYSTEM_CDC_STAGE* stage, DATABASE_SYSTEM_DB* db, DB_QUERY* data ) {
    if( stage && db && data ) {
        LOG_print( "\t· [CDC - STAGE::INSERTED (%d rows)]:\n", data->row_count );
        if( data->row_count == 0 ) {
            LOG_print( "\t\t- Action canceled.\n" );
        } else {
            CDC_StageGeneric( stage, &stage->inserted, db, data );
        }
    } else {
...
void DB_CDC_StageDeleted( DB_SYSTEM_CDC_STAGE *stage, DATABASE_SYSTEM_DB *db, DB_QUERY *data ) {
    if( stage && db && data ) {
        LOG_print( "\t· [CDC - STAGE::DELETED (%d rows)]:\n", data->row_count );
        if( data->row_count == 0 ) {
            LOG_print( "\t\t- Action canceled.\n" );
        } else {
            CDC_StageGeneric( stage, &stage->deleted, db, data );
        }
    } else {
...
void DB_CDC_StageModified( DB_SYSTEM_CDC_STAGE *stage, DATABASE_SYSTEM_DB *db, DB_QUERY *data ) {
    if( stage && db && data ) {
        LOG_print( "\t· [CDC - STAGE::MODIFIED (%d rows)]:\n", data->row_count );
        if( data->row_count == 0 ) {
            LOG_print( "\t\t- Action canceled.\n" );
        } else {
            CDC_StageGeneric( stage, &stage->modified, db, data );
        }
    } else {
