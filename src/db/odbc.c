...
static pthread_mutex_t      dbo_exec_mutex = PTHREAD_MUTEX_INITIALIZER;
...
int ODBC_exec(
	...
	pthread_mutex_lock( &dbo_exec_mutex );
	...
	if( db_connection->connection ) {
		...
		retcode = SQLPrepare( stmt, sql, strlen( sql ) );
	...
	pthread_mutex_unlock( &dbo_exec_mutex );
